...
helm upgrade -i --set image.name=$appAnalysisDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4202 -n $k8sNamespace app-analysis ./deploy/analysis/helm_chart

echo '>>>>>>>>>>>>>> Deploy auth image'
helm upgrade -i --set image.name=$appAuthDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4206 -n $k8sNamespace app-auth ./deploy/auth/helm_chart

echo '>>>>>>>>>>>>>> Deploy cashbook image'
helm upgrade -i --set image.name=$appCashbookDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4203 -n $k8sNamespace app-cashbook ./deploy/cashbook/helm_chart

echo '>>>>>>>>>>>>>> Deploy product image'
helm upgrade -i --set image.name=$appProductDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4205 -n $k8sNamespace app-product ./deploy/product/helm_chart

echo '>>>>>>>>>>>>>> Deploy summary image'
helm upgrade -i --set image.name=$appSummaryDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4201 -n $k8sNamespace app-summary ./deploy/summary/helm_chart

echo '>>>>>>>>>>>>>> Deploy user image'
helm upgrade -i --set image.name=$appUserDockerImage,image.tag=$dockerTag,replica=$k8sReplica,tcp_port=4204 -n $k8sNamespace app-user ./deploy/user/helm_chart
