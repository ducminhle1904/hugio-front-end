...
helm upgrade -i --set image.name=$appShellDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_shell ./deploy/shell/helm_chart

echo '>>>>>>>>>>>>>> Deploy analysis image'
helm upgrade -i --set image.name=$appAnalysisDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_analysis ./deploy/analysis/helm_chart

echo '>>>>>>>>>>>>>> Deploy auth image'
helm upgrade -i --set image.name=$appAuthDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_auth ./deploy/auth/helm_chart

echo '>>>>>>>>>>>>>> Deploy cashbook image'
helm upgrade -i --set image.name=$appCashbookDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_cashbook ./deploy/cashbook/helm_chart

echo '>>>>>>>>>>>>>> Deploy product image'
helm upgrade -i --set image.name=$appProductDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_product ./deploy/product/helm_chart

echo '>>>>>>>>>>>>>> Deploy summary image'
helm upgrade -i --set image.name=$appSummaryDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_summary ./deploy/summary/helm_chart

echo '>>>>>>>>>>>>>> Deploy user image'
helm upgrade -i --set image.name=$appUserDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_user ./deploy/user/helm_chart

echo '>>>>>>>>>>>>>> Deploy order image'
helm upgrade -i --set image.name=$appOrderDockerImage,image.tag=$dockerTag,replica=$k8sReplica -n $k8sNamespace app_order ./deploy/order/helm_chart
